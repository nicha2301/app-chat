...
const TextMsg = ({ el, menu }) => {
    ...
    return (
        <Stack direction='row' justifyContent={CheckMsg(el.senderId) ? 'start' : 'end'}>
            <Box p={1.5} sx={{
                backgroundColor: CheckMsg(el.senderId) ? theme.palette.background.default :
                    theme.palette.primary.main, borderRadius: 1.5, width: 'max-content'
            }}>
                <Typography variant='body2' color={CheckMsg(el.senderId) ? theme.palette.text : '#fff'}>
                    {el.content}
                </Typography>
            </Box>
            {menu && <MessageOptions />}
